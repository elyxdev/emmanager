...
class App(ctk.CTk):
    ...
    def load(self, name):
        ...
        if not messagebox.askyesno(
            "Usar modpack",
            f"Vas a usar '{name}'.\n\n"
            f"Se reemplazarán las carpetas mods, config y resourcepacks de:\n{self.service.game_dir}\n\n"
            "Lo que tengas ahora en ellas y no esté guardado en un modpack se perderá.\n¿Continuar?"
        ):
            return
        self.run_task(
            f"Preparando '{name}'...",
